...
curl ${var.verbose ? "-v" : "-sS"} -X POST "${self.triggers.AllocationUrl}?${self.triggers.AllocationQuery}" -H "Accept: application/json" -H "Authorization: Bearer $TOKEN"
...
TOKEN=$(az account get-access-token --query accessToken --output tsv)
curl ${var.verbose ? "-v" : "-sS"} -X DELETE "${self.triggers.AllocationUrl}" -H "Accept: application/json" -H "Authorization: Bearer $TOKEN"
